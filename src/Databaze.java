...
public class Databaze {
	...
	public void pridejStudenta() {
		...
		if (obor.equals("tli")) {
			...
			Stud_tel Student = new Stud_tel(jmeno, prijmeni, rok_narozeni);
			...
			Stud_kyber Student = new Stud_kyber(jmeno, prijmeni, rok_narozeni);
			DatabazeStudentu.add(Student);
			System.out.println("Student uspesne pridan");
		} else {
	...
	public void nactiStudenta(String jmenoSouboru) {
		...
		try {
			...
			if (castiRadku[0].equals("tel")) {
				student = new Stud_tel(castiRadku[1], castiRadku[2], Integer.parseInt(castiRadku[3]));
				DatabazeStudentu.add(student);
			}
			else if (castiRadku[0].equals("kyber")) {
				student = new Stud_kyber(castiRadku[1], castiRadku[2], Integer.parseInt(castiRadku[3]));
				DatabazeStudentu.add(student);
			}
	...
	public void nactiDatabazi() {
		...
		while(end) {
			if ((student = pripojeni.nactiStudenta(rs)) != null) {
				DatabazeStudentu.add(student);
			}
			else {
				end = false;	
